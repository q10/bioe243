...
class Particle:
    ...
    def enclose_to_boundary(self):
        while self.x < -self.boundary:
            self.x += 2*self.boundary
        while self.x > self.boundary:
            self.x -= 2*self.boundary

        while self.y < -self.boundary:
            self.y += 2*self.boundary
        while self.y > self.boundary:
            self.y -= 2*self.boundary

        while self.z < -self.boundary:
            self.z += 2*self.boundary
        while self.z > self.boundary:
            self.z -= 2*self.boundary
...
f = open('SILVER.RESULTS','w')

a = SCCluster(metal="silver")
for i in range(10000):
    a.MD_step()
    
print >>f, "Potential Energy = " + str(a.system_potential_energy) + "\n" + \
      "Kinetic Energy = " + str(a.system_kinetic_energy) + "\n" + \
      "Total Energy = " + str(a.system_total_energy) + "\n" + \
      "Current Temperature = " + str(a.current_temperature) + "\n" + \
      "Self Diffusion Constant = " + str(a.calculate_diffusitivity()) + "\n" + \
      "Binding Energy = " + str(a.system_total_energy/a.N) + "\n\n" + \
