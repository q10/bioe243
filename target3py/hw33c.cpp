...
float BestConfiguration [108][3];		// Position of best configurations of particles
...
void GetInitPositionsAndInitVelocities(float SetpointTemp){
	...
	while (!feof(filePosition)) {	
			...
			BestConfiguration[i][0] = xi;
			BestConfiguration[i][1] = yi;
			BestConfiguration[i][2] = zi;
...
void BussiThermostat(float deltaT, float CurrentTemp, float SetpointTemp) {
